...
- When automatic dispatch is enabled, opening or pushing to a pull request of the opted-in repository reserves and dispatches a local run without any command. It is an outbound poll of that one repository, never a listener or a relay; a new commit supersedes the older approval of the same pull request, and work belonging to a different pull request is never cancelled. `run-local` remains the explicit, always-available path.
