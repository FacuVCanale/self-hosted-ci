...
def timestamp(value: object) -> datetime:
    if not isinstance(value, str) or not value.endswith("Z"): raise ValueError("timestamp must be UTC")
    parsed = datetime.fromisoformat(value[:-1] + "+00:00")
    if parsed.tzinfo is None: raise ValueError("timestamp requires timezone")
    return parsed.astimezone(timezone.utc)

def validate(payload: object, expected_sid: str, expected_distro: str, now: datetime) -> tuple[int, str]:
    if not isinstance(payload, dict) or set(payload) != TOP_LEVEL or payload.get("schema_version") != 2: return 5, "invalid_snapshot_schema"
    try:
        generated, expires = timestamp(payload["generated_at"]), timestamp(payload["expires_at"])
        if not isinstance(payload["install_nonce"], str) or not UUID.fullmatch(payload["install_nonce"]): raise ValueError()
        producer, host, distro = payload["producer"], payload["host"], payload["distro"]
        runner, services, heartbeat = payload["runner"], payload["services"], payload["heartbeat"]
        boundary, eligibility = payload["boundary"], payload["eligibility"]
        if not isinstance(producer, dict) or set(producer) != {"windows_sid", "account", "distro"}: raise ValueError()
        if producer["windows_sid"] != expected_sid or not SID.fullmatch(producer["windows_sid"]): raise ValueError()
        if producer["account"] != "selfhosted-ci-svc" or producer["distro"] != expected_distro: raise ValueError()
        if not isinstance(host, dict) or set(host) != {"service_identity_verified", "services"} or host["service_identity_verified"] is not True: raise ValueError()
        if not isinstance(host["services"], dict) or set(host["services"]) != HOST_SERVICES: raise ValueError()
        for state in host["services"].values():
            if not isinstance(state, dict) or set(state) != {"installed", "status"} or not isinstance(state["installed"], bool) or not isinstance(state["status"], str): raise ValueError()
        ...
        if heartbeat["age_seconds"] is not None and (not isinstance(heartbeat["age_seconds"], int) or heartbeat["age_seconds"] < 0): raise ValueError()
        observed = timestamp(heartbeat["observed_at"]) if heartbeat["observed_at"] is not None else None
        if heartbeat["status"] in {"fresh", "stale"}:
            if observed is None or not isinstance(heartbeat["age_seconds"], int): raise ValueError()
            expected_age = max(0, int((generated - observed).total_seconds()))
            if abs(heartbeat["age_seconds"] - expected_age) > 2 or observed > generated + timedelta(seconds=2): raise ValueError()
            if (heartbeat["status"] == "fresh") != (heartbeat["age_seconds"] <= heartbeat["max_age_seconds"]): raise ValueError()
        elif observed is not None or heartbeat["age_seconds"] is not None:
            raise ValueError()
        if not isinstance(boundary, dict) or set(boundary) != {"activation_approved", "network_policy_enabled"}: raise ValueError()
        if boundary["activation_approved"] not in (True, False, None) or boundary["network_policy_enabled"] not in (True, False, None): raise ValueError()
        if not isinstance(eligibility, dict) or set(eligibility) != {"eligible_for_local_ci", "blocking_reasons"}: raise ValueError()
        if not isinstance(eligibility["eligible_for_local_ci"], bool): raise ValueError()
        if not isinstance(eligibility["blocking_reasons"], list) or not all(isinstance(item, str) and item for item in eligibility["blocking_reasons"]): raise ValueError()
        if expires <= generated or expires - generated > timedelta(seconds=300): raise ValueError()
        if payload["probe_error"] is not None and (not isinstance(payload["probe_error"], str) or not payload["probe_error"] or len(payload["probe_error"]) > 1024): raise ValueError()
        if payload["probe_error"] is not None and (eligibility["eligible_for_local_ci"] or "supervisor_probe_failed" not in eligibility["blocking_reasons"]): raise ValueError()
        if len(set(eligibility["blocking_reasons"])) != len(eligibility["blocking_reasons"]): raise ValueError()
        if payload["probe_error"] is not None:
            ...
            for name, state in services.items():
                if state["active"] != "active": expected_blockers.add(f"service_not_active:{name}")
                if state["enabled"] not in ENABLED_STATES[name]: expected_blockers.add(f"service_not_enabled:{name}")
            if host["services"]["sshd"]["status"] != "running": expected_blockers.add("host_service_not_running:sshd")
            if host["services"]["wsl"]["status"] != "running" and host["services"]["lxss_manager"]["status"] != "running": expected_blockers.add("host_service_not_running:wsl")
        if eligibility["blocking_reasons"] != sorted(expected_blockers): raise ValueError()
        if eligibility["eligible_for_local_ci"] != (not expected_blockers): raise ValueError()
    except (KeyError, TypeError, ValueError): return 5, "invalid_snapshot_contract"
    if generated > now + timedelta(seconds=30): return 5, "snapshot_from_future"
    if now >= expires: return 4, "snapshot_expired"
    if not eligibility["eligible_for_local_ci"]: return 3, "local_ci_ineligible"
    if (
...
def main() -> int:
    ...
    if not SID.fullmatch(args.expected_sid): print("invalid_expected_sid"); return 2
    try: payload = json.loads(args.snapshot.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError): print("invalid_snapshot_json"); return 5
    code, status = validate(payload, args.expected_sid, args.expected_distro, datetime.now(timezone.utc))
    print(json.dumps({"status": status, "snapshot": payload}, sort_keys=True, separators=(",", ":")))
    return code
