...
class Observer:
    ...
    def run(self, run_id):
        return self._call(self.client.run, run_id)

    def jobs(self, run_id):
        return self._call(self.client.jobs, run_id)
...
def load_client():
    ...
    if (
        ...
        or not private_key.startswith("/etc/self-hosted-ci/secrets/")
        or ".." in Path(private_key).parts
    ):
    ...
    client = WorkerGitHubClient(
        authority, RootPrivateKeySigner.from_file(Path(private_key)), UrllibTransport()
    )
...
def main(argv=None) -> int:
    ...
    try:
        ...
        PilotTerminalMonitor(
            Observer(client, token),
            Broker(args.runner_label),
            time.sleep,
            time.monotonic,
        ).monitor(
