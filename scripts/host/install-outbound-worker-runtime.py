...
def load_config(path: Path) -> dict[str, Any]:
    try:
        value = json.loads(
            path.read_text(encoding="utf-8"), object_pairs_hook=_exact_object
        )
    ...
    if (
        ...
        or value["permissions"] != EXACT_PERMISSIONS
    ):
    ...
    if not isinstance(workflow_id, str) or not re.fullmatch(
        r"[A-Za-z0-9_.-]+\.ya?ml", workflow_id
    ):
    ...
    if (
        not isinstance(value["default_branch"], str)
        or not re.fullmatch(r"[A-Za-z0-9._/-]+", value["default_branch"])
        or value["default_branch"].startswith("/")
    ):
    ...
    if (
        github_key.parent != SECRET_ROOT
        or allocation_key.parent != SECRET_ROOT
        or github_key == allocation_key
    ):
        raise InstallError(
            "worker private keys must be distinct direct children of the managed secrets directory"
        )
    for field in ("gatestore_file", "approval_store_file", "worker_state_file"):
        state_path = PurePosixPath(str(value[field]))
        if state_path.parent != STATE_ROOT or state_path.suffix not in {
            ".db",
            ".sqlite3",
        }:
    ...
    if (
        len(
            {
                value["gatestore_file"],
                value["approval_store_file"],
                value["worker_state_file"],
            }
        )
        != 3
    ):
...
def _private_key(path: Path, expected: str) -> None:
    ...
    if expected == "GitHub App":
        if not isinstance(key, rsa.RSAPrivateKey) or key.key_size < 2048:
            raise InstallError(
                "GitHub App private key must be RSA 2048-bit or stronger"
            )
...
def _secure_file(
    path: Path, *, executable: bool = False, expected_uid: int = 0
) -> None:
    try:
        info = os.lstat(path)
    ...
    if (
        ...
        raise InstallError(
            f"runtime file must be owned by uid {expected_uid}, regular, single-link mode {expected_mode:04o}: {path}"
        )


def _atomic_install(
    source: Path, target: Path, mode: int, *, uid: int = 0, gid: int = 0
) -> None:
...
"""
    ...
    if result.returncode != 0 or result.stdout.strip() != "isolated-import-smoke-ok":
        raise InstallError(
            f"installed Python import closure failed: {result.stderr.strip()}"
        )


def verify_runtime(
    *, prefix: Path = Path("/"), expected_uid: int = 0, require_ready: bool = True
) -> dict[str, Any]:
    ...
    if require_ready:
        ...
        try:
            ready = json.loads(
                ready_target.read_text(encoding="utf-8"),
...
def install_runtime(
    ...
    *,
    ...
    _atomic_install(
        config_source, config_target, 0o600, uid=expected_uid, gid=target_gid
    )
    _atomic_install(
        github_key_source, github_target, 0o600, uid=expected_uid, gid=target_gid
    )
    _atomic_install(
    ...
    with tempfile.NamedTemporaryFile(
        "w", encoding="utf-8", dir=ready_target.parent, delete=False
    ) as output:
        json.dump(sentinel, output, sort_keys=True, separators=(",", ":"))
        output.write("\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--acknowledge-local-smoke-has-no-github-proof", action="store_true"
    )
    ...
    if args.verify:
        try:
            ...
            print(
                f"outbound worker runtime verification blocked: {exc}", file=sys.stderr
            )
            return 2
    plan = {
        "mode": "plan",
    ...
    try:
        ...
        if not (
            args.acknowledge_install_root_only_worker_secrets
            and args.acknowledge_local_smoke_has_no_github_proof
        ):
