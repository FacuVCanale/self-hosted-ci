...
GARM_STATE_KEYS = {
    "schema_version", "manager_configured", "provider_configured",
    "image_configured", "broker_configured", "zero_scale_sets", "image", "targets", "garm_cli_home",
}
BROKER_CONFIG = Path("/etc/self-hosted-ci/garm/allocation-broker.json")
LIVE_JOB_VERIFIER = Path("/usr/local/libexec/self-hosted-ci/github-live-job-verifier.py")
...
def regular_root_file(path: Path, *, executable: bool = False, maximum_size: int | None = None) -> bool:
    try:
        details = path.lstat()
    except OSError:
        return False
...
def root_secret_file(path: Path) -> bool:
    try:
        ...
        return False
    return regular_root_file(path, maximum_size=4096) and stat.S_IMODE(details.st_mode) == 0o600


def garm_configuration_state() -> dict[str, bool | None]:
    unknown = {
        "manager_configured": None, "provider_configured": None,
        "image_configured": None, "broker_configured": None, "zero_scale_sets": None,
    }
    ...
    try:
        ...
        return unknown
    if not isinstance(payload, dict) or set(payload) != GARM_STATE_KEYS or payload.get("schema_version") != 3:
        return unknown
    values = {key: payload[key] for key in ("manager_configured", "provider_configured", "image_configured", "broker_configured", "zero_scale_sets")}
    if any(type(value) is not bool for value in values.values()):
        return unknown
    cli_home = payload["garm_cli_home"]
    if cli_home != GARM_RUNTIME_HOME:
        return unknown
    if not regular_root_file(GARM_CLI, executable=True) or not regular_root_file(GARM_SESSION_HELPER, executable=True):
        return unknown
    if any(not root_secret_file(path) for path in GARM_SESSION_SECRETS):
        return unknown
    if not regular_root_file(BROKER_CONFIG, maximum_size=65536) or not regular_root_file(LIVE_JOB_VERIFIER, executable=True):
        return unknown
    try: broker = json.loads(BROKER_CONFIG.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError): return unknown
    if broker.get("targets") != payload["targets"] or broker.get("live_job_verifier") != str(LIVE_JOB_VERIFIER):
        return unknown
    ...
    try:
        ...
        inventories = [query("scaleset", "list", target["entity_flag"], target["entity_id"]) for target in payload["targets"].values()]
        image_result = subprocess.run(
            ["incus", "image", "info", payload["image"]["alias"], "--project", "ci-jit", "--format", "json"],
        ...
        return unknown
    if not isinstance(controller, dict) or controller.get("callback_url") != "http://10.254.0.1:8080/api/v1/callbacks" or controller.get("metadata_url") != "http://10.254.0.1:8080/api/v1/metadata":
        return unknown
    if not isinstance(providers, list) or len(providers) != 1 or providers[0].get("name") != "incus_ci_jit" or providers[0].get("type") != "external":
        return unknown
    if any(inventory != [] for inventory in inventories):
        return unknown
    image = payload["image"]
    if not isinstance(image, dict) or set(image) != {"alias", "fingerprint"} or live_image.get("fingerprint") != image["fingerprint"] or image["alias"] not in [item.get("name") for item in live_image.get("aliases", [])]: return unknown
    ...
    values["provider_configured"] = bool(
        values["provider_configured"] and regular_root_file(GARM_PROVIDER, executable=True)
    )
    values["broker_configured"] = bool(values["broker_configured"])
    values["zero_scale_sets"] = bool(values["zero_scale_sets"] and all(inventory == [] for inventory in inventories))
    return values
...
def persistent_registration_present() -> bool | None:
    ...
    for name in (".runner", ".credentials", ".credentials_rsaparams"):
        try:
            ...
            return None
    unit_roots = (Path("/etc/systemd/system"), Path("/usr/lib/systemd/system"), Path("/lib/systemd/system"))
    for root in unit_roots:
        try:
            if root.is_dir() and any(
                entry.name.startswith("actions.runner.") and entry.name.endswith(".service")
...
def observe(distro: str, maximum_age: int) -> dict[str, object]:
    ...
    runner = {
        "mode": "garm-jit",
        "manager": {"configured": configured["manager_configured"], "state": manager_state},
        "provider": {"configured": configured["provider_configured"]},
        "image": {"configured": configured["image_configured"]},
        "broker": {"configured": configured["broker_configured"], "state": broker_state},
    ...
    if configured["manager_configured"] is not True: reasons.append("garm_manager_not_configured")
    if configured["provider_configured"] is not True: reasons.append("garm_provider_not_configured")
    if configured["image_configured"] is not True: reasons.append("runner_image_not_configured")
    if configured["broker_configured"] is not True: reasons.append("allocation_broker_not_configured")
    if configured["zero_scale_sets"] is not True: reasons.append("transient_scale_sets_not_clean")
    if persistent_registration is not False: reasons.append("persistent_runner_registration_not_clean")
    if idle_instances is None: reasons.append("jit_instances_not_observable")
    elif idle_instances != 0: reasons.append("idle_jit_instances_present")
    ...
    for name in SERVICES:
        ...
        if services[name]["enabled"] not in ({"enabled"} if name in {"incus.service", "self-hosted-ci-garm.service", "self-hosted-ci-allocation-broker.service", "self-hosted-ci-health-heartbeat.timer"} else {"enabled", "static"}):
