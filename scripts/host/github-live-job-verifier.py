...
def load_credentials(path: Path = CONFIG_PATH) -> tuple[int, int, rsa.RSAPrivateKey]:
    ...
    if (
        ...
        or not key_name.startswith("/etc/self-hosted-ci/")
    ):
...
def validate_request(value: Any) -> dict[str, Any]:
    ...
    for field in ("workflow_job_id", "run_id", "repository_id"):
        if not isinstance(value[field], str) or not _POSITIVE_INTEGER.fullmatch(
            value[field]
        ):
            raise VerificationError(
                f"{field} must be a canonical positive integer string"
            )
    if (
        not isinstance(value["run_attempt"], int)
        or isinstance(value["run_attempt"], bool)
        or value["run_attempt"] < 1
    ):
        raise VerificationError("run_attempt must be a positive integer")
    if not isinstance(value["repository"], str) or not _REPOSITORY.fullmatch(
        value["repository"]
    ):
        raise VerificationError("repository is invalid")
    if not isinstance(value["dispatch_sha"], str) or not _SHA1.fullmatch(
        value["dispatch_sha"]
    ):
        raise VerificationError("dispatch_sha is invalid")
    workflow = (
        _WORKFLOW_REF.fullmatch(value["workflow_ref"])
        if isinstance(value["workflow_ref"], str)
        else None
    )
    ...
    if (
        ...
        or any(
            not isinstance(label, str) or not label or len(label) > 63
            for label in labels
        )
...
class GitHubAPI:
    ...
    def json(
        ...
        request = urllib.request.Request(
            API_ROOT + path, data=encoded, method=method, headers=headers
        )
        try:
...
def _require_exact_repository(value: Any, request: Mapping[str, Any]) -> None:
    if (
        ...
        or value.get("id") != int(request["repository_id"])
        or value.get("full_name") != request["repository"]
    ):
    ...
    *,
    ...
    token_response = api.json(
        ...
        app_jwt,
        {
            ...
            "repository_ids": [int(request["repository_id"])],
        },
    )
    if not isinstance(token_response, dict) or set(token_response) < {
        "token",
        "expires_at",
        "permissions",
        "repositories",
    ...
    if (
        ...
        raise VerificationError(
            "installation token permissions are not read-only Actions"
        )
    ...
    try:
        expiry = datetime.fromisoformat(
            token_response["expires_at"].replace("Z", "+00:00")
        )
