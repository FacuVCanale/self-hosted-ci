...
def load_evidence_installer():
    spec = importlib.util.spec_from_file_location("install_wsl_jit_evidence", EVIDENCE_INSTALLER)
...
class IncusGarmTlsTests(unittest.TestCase):
    def test_installer_is_plan_only_by_default_and_apply_is_guarded(self) -> None:
        syntax = subprocess.run(["bash", "-n", str(INSTALLER)], text=True, capture_output=True)
        ...
        apply = subprocess.run(["bash", str(INSTALLER), "--apply"], text=True, capture_output=True)
    ...
    def test_provisioning_installs_tls_boundary_but_keeps_activation_inert(self) -> None:
        source = PROVISION.read_text()
        self.assertIn('install-incus-garm-tls.sh" --apply', source)
        self.assertIn('--provider-template "/usr/local/share/self-hosted-ci/garm-provider-incus.toml"', source)
        self.assertIn("--acknowledge-loopback-tls-boundary", source)
        self.assertIn('bash "${repo_root}/scripts/host/install-runner-network-runtime.sh"', source)
        self.assertIn("systemctl start self-hosted-ci-boundary-verify.service", source)
        self.assertIn("systemctl is-active --quiet self-hosted-ci-boundary-verify.service", source)
        self.assertIn('rm -f "${TARGET_ROOT}/ACTIVATION_APPROVED"', source)
        for transaction_script in (
            "activate-garm-jit.sh", "deactivate-garm-jit.sh", "garm-jit-transaction-lib.sh",
        ):
            self.assertIn(transaction_script, source)
            self.assertNotIn(f'/usr/local/lib/self-hosted-ci/{transaction_script}" --', source)
        self.assertIn(
    ...
    def test_verified_evidence_is_installed_from_exact_signed_refs(self) -> None:
        ...
        self.assertIn('installed runner-boundary evidence failed post-install verification', provision)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            bundle.write_text(json.dumps({
                ...
                "measurements": {"artifacts": [{
                    "ref": "evidence/incus.json", "uid": 0, "gid": 0,
                    "mode": "0640", "sha256": "0" * 64,
            ...
            self.assertEqual(referenced.read_bytes(), (target / "host-evidence/evidence/incus.json").read_bytes())
            self.assertFalse((target / "host-evidence/reviewer-private-key.pem").exists())
            self.assertEqual(bundle.read_bytes(), (target / "runner-boundary-v2.json").read_bytes())
            self.assertFalse(any(path.name.startswith(".host-evidence.") for path in target.iterdir()))
