...
class AppAuthorityTests(unittest.TestCase):
    def test_s62_exact_positive_and_negative_permissions(self) -> None:
        ...
        self.assertTrue(
            auth.permits(
                "checks:create", repository=auth.repository, installation_id=222
            )
        )
        self.assertTrue(
            auth.permits(
                "checks:update", repository=auth.repository, installation_id=222
            )
        )
        ...
        self.assertFalse(
            auth.permits(
                "checks:update", repository=f"{OWNER}/other", installation_id=222
            )
        )
    ...
    def test_s63_only_exact_control_identity_can_mint_narrow_memory_token(self) -> None:
        ...
        self.assertEqual(
            mint_request(ttl_seconds=3599),
            auth.mint(identity("reconciler"), mint_request(ttl_seconds=3599)),
        )
        for bad_identity in (
            identity("child"),
            RuntimeIdentity("coordinator", "other/workflow", "refs/heads/main"),
            RuntimeIdentity(
                "coordinator", identity().workflow_identity, "refs/pull/42/merge"
            ),
            RuntimeIdentity(
                "coordinator", identity().workflow_identity, "refs/heads/main", False
            ),
        ):
    ...
    def test_s63_ttl_scope_permissions_and_storage_boundaries(self) -> None:
        ...
        invalid = (
            {"ttl_seconds": 3601},
            {"repository": f"{OWNER}/other"},
            {"installation_id": 223},
            {"permissions": {"checks": "write"}},
            {"export_to_children": True},
            {"persist": True},
            {"masked": False},
            {"memory_only": False},
        )
    ...
    def test_s64_rotation_requires_new_key_then_old_revokes(self) -> None:
        old = authority()
        new = authority(
            key_version=2, key_fingerprint="f" * 64, rotated_at=NOW + timedelta(days=1)
        )
        self.assertIs(new, old.rotated_to(new))
        revoked = old.revoked()
        self.assertFalse(
            revoked.permits(
                "checks:update", repository=old.repository, installation_id=222
            )
        )
...
class DispatchAndProtocolTests(unittest.TestCase):
    def test_two_phase_observation_requires_exact_run_job_and_reserved_label(
        self,
    ) -> None:
        label = "wsl-jit-" + "1" * 32
        observed = parse_observed_workflow_job(
            777,
            label,
            {"id": 777, "run_attempt": 2, "head_sha": "f" * 40},
            {
                "total_count": 2,
                "jobs": [
                    {
        ...
        self.assertEqual(
            (777, 2, 888), (observed.run_id, observed.run_attempt, observed.job_id)
        )
        invalid = (
            {"total_count": 0, "jobs": []},
            {
    ...
    def test_s45_dispatch_is_pinned_and_consumes_exact_http_200_run_id(self) -> None:
        ...
        invalid_responses = (
            ...
            (
                ...
                {
                    **receipt,
                    "run_url": "https://api.github.com/repos/example-owner/other/actions/runs/777",
                },
            ),
            (
                200,
                {
                    **receipt,
                    "html_url": f"https://github.com/{REPOSITORY}/actions/runs/778",
                },
            ),
        )
        for status, body in invalid_responses:
            with (
                ...
                self.assertRaises(ProtocolFailure),
            ):
    ...
    def test_protocol_rejects_unknown_missing_and_backend_crossing(self) -> None:
        ...
        unknown = protocol()
        unknown["guess_run_from_list"] = True
        cases.append(unknown)
        crossed = protocol("github")
        crossed["execution_trust_mode"] = "exact-sha-attestation"
        cases.append(crossed)
        leaked = protocol("github")
        leaked["attestation_id"] = "foreign"
        cases.append(leaked)
        for value in cases:
    ...
    def test_s13_base_or_merge_movement_invalidates_same_head(self) -> None:
        ...
        for base, merge in (("9" * 40, MERGE), (BASE, "9" * 40)):
            with (
                self.subTest(base=base, merge=merge),
                self.assertRaises(ProtocolFailure),
            ):
                package.assert_current_tuple(
                    repository_id=123,
...
class ClaimAndGateSourceTests(unittest.TestCase):
    ...
    def test_s31_s43_s51_s52_only_dedicated_explicit_app_check_satisfies(self) -> None:
        self.assertTrue(
            validate_ci_gate_source(
                self.source, authority=self.auth, check_target_sha=MERGE
            )
        )
        mutations = (
            {"app_id": 999},
            {"kind": "commit_status"},
            {"explicit_custom_check": False},
        ...
        for mutation in mutations:
            ...
            with self.subTest(mutation=mutation):
                self.assertFalse(
                    validate_ci_gate_source(
                        event, authority=self.auth, check_target_sha=MERGE
                    )
                )
    ...
    def test_s33_main_ci_is_not_a_pr_gate_source(self) -> None:
        ...
        self.assertFalse(
            validate_ci_gate_source(
                push_ci, authority=self.auth, check_target_sha=MERGE
            )
        )

    def test_s07_formal_claim_requires_exact_returned_run_started_job_labels_and_runner(
        self,
    ) -> None:
        package = ProtocolPackage.from_mapping(protocol())
        run = {
        ...
        job = {
            "id": 555,
            "name": "local-attempt",
            "started_at": "2026-08-26T12:10:00Z",
        ...
        self.assertTrue(
            validate_formal_claim(
                ...
                run=run,
                jobs=[job],
                expected_workflow_id="child.yml",
                expected_job_name="local-attempt",
                expected_labels=["jit-42", "self-hosted"],
                expected_runner_id="runner-unique",
            )
        )
        invalid = []
        wrong_run = deepcopy(run)
        wrong_run["id"] = 999
        invalid.append((wrong_run, [job], 444))
        queued = deepcopy(job)
        queued["started_at"] = None
        invalid.append((run, [queued], 444))
        late = deepcopy(job)
        late["started_at"] = "2026-08-26T12:10:00.000001Z"
        invalid.append((run, [late], 444))
        labels = deepcopy(job)
        labels["labels"] = ["self-hosted"]
        invalid.append((run, [labels], 444))
        reused = deepcopy(job)
        reused["runner"]["fresh"] = False
        invalid.append((run, [reused], 444))
        invalid.append((run, [job, job], 444))
        for bad_run, bad_jobs, receipt in invalid:
            with (
                self.subTest(run=bad_run, jobs=bad_jobs),
                self.assertRaises(ProtocolFailure),
            ):
                validate_formal_claim(
                    package=package,
                    dispatch_run_id=receipt,
                    run=bad_run,
                    jobs=bad_jobs,
                    expected_workflow_id="child.yml",
                    expected_job_name="local-attempt",
...
class HostedConclusionTests(unittest.TestCase):
    def test_hosted_predicate_checks_winner_lease_tuple_command_job_deadline_and_app_not_attestation(
        self,
    ) -> None:
        ...
        hosted = {
            ...
            "check_target_sha": MERGE,
            "canonical_command_digest": DIGEST,
            "workflow_id": "hosted.yml",
            "workflow_ref": "main",
            "run_id": 700,
            "job_id": 701,
            "trustworthy_github_hosted": True,
            "terminal_at": "2026-08-26T12:20:00Z",
            "execution_deadline": "2026-08-26T12:20:00Z",
            "check_source": source,
        ...
        self.assertTrue(
            hosted_conclusion_allowed(
                package=package, gate=gate, hosted=hosted, authority=auth, now=NOW
            )
        )
        for target, key, value in (
            ...
            (gate, "owner", 901),
            (gate, "generation", 8),
            (hosted, "tested_sha", SHA),
            (hosted, "canonical_command_digest", "f" * 64),
            (hosted, "run_id", 999),
            (hosted, "terminal_at", "2026-08-26T12:20:00.000001Z"),
        ):
