...
class HostHealthScriptTests(unittest.TestCase):
    ...
    def test_supervisor_enables_reader_only_after_sftp_fence_and_uninstall_disables_it(self) -> None:
        ...
        self.assertIn('if ($reader.Enabled) { throw "health reader must be disabled before SFTP activation" }', install)
        ...
        verifier = install[install.index("function Assert-SftpOnlyConfiguration"):install.index("if ($env:OS -ne", install.index("function Assert-SftpOnlyConfiguration"))]
        ...
        rollback_stop = install.index("Stop-Service -Name sshd -ErrorAction SilentlyContinue", rollback)
        rollback_disable = install.index("Disable-LocalUser -Name $reader.Name", rollback_stop)
        rollback_restore = install.index("Copy-Item -LiteralPath $SshdBackup -Destination $SshdConfig -Force", rollback_disable)
        rollback_start = install.index("Start-Service -Name sshd -ErrorAction Stop", rollback_restore)
        self.assertIn("refusing to restore unrestricted sshd configuration while reader is enabled", install[rollback_disable:rollback_restore])
