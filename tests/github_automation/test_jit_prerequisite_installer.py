...
class JitPrerequisiteInstallerTests(unittest.TestCase):
    ...
    def test_payload_pins_prerequisites_and_remains_inert(self) -> None:
        ...
        for token in (
            ...
            "garm_cli_version='0.2.1'", "983fa54557f3f5ce3aa1eeb2387499f5f823d14512a0559ba888667bc3b3e88e",
            "garm-cli-linux-amd64.tgz", 'tar -tzf "${tx}/garm-cli.tgz"', "garm-cli version", "root:root:755",
            "garm_provider_incus_version='0.1.5'", "1489b5f9b3f01528e338c604c13dabe8321ed6f1bc6de77c7344119d7731c43f",
            "garm-provider-incus-linux-amd64.tgz", 'tar -tzf "${tx}/garm-provider-incus.tgz"', "/usr/local/libexec/garm/garm-provider-incus",
            '"incus=${incus_version}"', "dnsmasq-base", "dpkg-query -W -f='${Status}' dnsmasq-base", "apt-get remove -y dnsmasq", "command -v dnsmasq", "dnsmasq --version", "dnsmasq.service", "e2fsprogs", "util-linux", "command -v mkfs.ext4", "command -v losetup", "command -v tune2fs", "command -v findmnt", "command -v mountpoint", "command -v systemd-escape", "command -v blockdev", "command -v blkid", "grep -qw ext4 /proc/filesystems", "loop devices", "apt-mark hold incus", "sha256sum --check --status",
            "nftables", "squid", "systemctl disable --now squid.service nftables.service",
            '"dnsmasq_base_installed":true', '"dnsmasq_service_absent":true', '"nftables_installed":true', '"squid_installed":true', '"distribution_network_services_disabled":true', '"ext4_tools_installed":true', '"ext4_kernel_supported":true', '"loop_devices_supported":true',
            "useradd --system", "garm-manager", "passwd --lock garm-manager", "gpasswd --delete garm-manager incus-admin", "garm-manager retains forbidden incus-admin membership", '"garm_manager_incus_admin":false',
    ...
    def test_garm_021_config_template_uses_current_schema_and_placeholders(self) -> None:
        source = GARM_CONFIG.read_text(encoding="utf-8")
        parsed = tomllib.loads(source)
        for token in (
            "[default]", "enable_webhook_management = false", "[logging]", "[metrics]",
            "[apiserver]", 'bind = "127.0.0.1"', "[apiserver.webui]",
            "[database]", 'backend = "sqlite3"', "[database.sqlite3]",
            'db_file = "/var/lib/self-hosted-ci/garm/garm.db"', "[[provider]]", 'provider_type = "external"',
            '[provider.external]', 'provider_executable = "/usr/local/libexec/garm/garm-provider-incus"',
            'config_file = "/etc/self-hosted-ci/garm/garm-provider-incus.toml"', 'time_to_live = "24h"',
        ):
            self.assertIn(token, source)
        ...
        self.assertEqual("/var/lib/self-hosted-ci/garm/garm.db", parsed["database"]["sqlite3"]["db_file"])
    ...
    def test_versions_and_deadlines_are_policy_pinned(self) -> None:
        ...
        self.assertIn('$ExpectedGarmCliSha256 = "983fa54557f3f5ce3aa1eeb2387499f5f823d14512a0559ba888667bc3b3e88e"', source)
        self.assertIn('$ExpectedGarmProviderIncusSha256 = "1489b5f9b3f01528e338c604c13dabe8321ed6f1bc6de77c7344119d7731c43f"', source)
