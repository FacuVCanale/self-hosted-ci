...
class HostedReusableGateWorkflowTests(unittest.TestCase):
    ...
    def test_quality_builds_exact_local_ort_merge_without_oidc_or_file_protocol(self) -> None:
        ...
        self.assertIn("ci-gate-toolchain-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}", quality)
        self.assertIn('test "$(git --version)" = "$EXPECTED_GIT_VERSION"', quality)
        self.assertIn('test ! -L "$TOOLCHAIN_SENTINEL"', quality)
        self.assertIn('TOOLCHAIN_SENTINEL_DIGEST: ${{ steps.toolchain.outputs.sentinel_digest }}', quality)
