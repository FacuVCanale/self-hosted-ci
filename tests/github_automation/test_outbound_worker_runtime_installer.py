...
def load_installer():
    spec = importlib.util.spec_from_file_location("install_outbound_worker_runtime", INSTALLER)
...
def load_stager():
    spec = importlib.util.spec_from_file_location("stage_wsl_jit_live_contract_for_runtime", STAGER)
    module = importlib.util.module_from_spec(spec)
    assert spec.loader
    spec.loader.exec_module(module)
    return module
...
class OutboundWorkerRuntimeInstallerTests(unittest.TestCase):
    def config(self) -> dict:
        return {
            "schema_version": 1, "mode": "ci-jit-pilot", "app_id": 123,
            "app_slug": "self-hosted-ci-worker", "installation_id": 456,
            "repository": "OWNER/REPO", "repository_id": 789,
            ...
            "permissions": {"metadata": "read", "pull_requests": "read", "actions": "write"},
            ...
            "approval_ttl_seconds": 240, "poll_seconds": 15, "request_timeout_seconds": 30,
        }

    def stage_runtime(self, root: Path) -> None:
        stager = load_stager()
        for relative, target, mode, _kind, _component in stager.PUBLIC_ARTIFACTS:
            if not target.startswith("/usr/local/lib/self-hosted-ci/github_automation/"):
                continue
    ...
    def sources(self, root: Path) -> tuple[Path, Path, Path]:
        ...
        github_key.write_bytes(private_pem(rsa.generate_private_key(public_exponent=65537, key_size=2048)))
    ...
    def test_default_is_inert_plan_and_apply_is_ack_guarded(self) -> None:
        ...
        apply = subprocess.run([str(INSTALLER), "--apply"], text=True, capture_output=True)
        self.assertNotEqual(0, apply.returncode)
        self.assertIn("--apply must run as root" if os.geteuid() else "both explicit acknowledgements", apply.stderr)

    def test_exact_config_rejects_unknown_fields_and_unsafe_paths(self) -> None:
        installer = load_installer()
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "config.json"
            for mutation in ("unknown", "secret-path", "database-crossing", "full-mode"):
                value = self.config()
                if mutation == "unknown": value["extra"] = True
                elif mutation == "secret-path": value["github_app_private_key_file"] = "/tmp/key.pem"
                elif mutation == "database-crossing": value["worker_state_file"] = value["approval_store_file"]
                else: value["mode"] = "ci-gate-full"
                path.write_text(json.dumps(value), encoding="utf-8")
                with self.subTest(mutation=mutation), self.assertRaises(installer.InstallError):
                    installer.load_config(path)

    def test_apply_is_idempotent_and_creates_sentinel_only_after_local_smoke(self) -> None:
        installer = load_installer()
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            for _ in range(2):
                result = installer.install_runtime(
                    config, github_key, allocation_key, prefix=root, expected_uid=os.getuid(),
                )
                self.assertTrue(result["runtime_ready"])
                self.assertFalse(result["external_calls"])
                self.assertFalse(result["dispatch"])
                verified = installer.verify_runtime(prefix=root, expected_uid=os.getuid())
    ...
    def test_failed_apply_removes_existing_readiness(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            config, github_key, allocation_key = self.sources(root)
            allocation_key.write_bytes(private_pem(rsa.generate_private_key(public_exponent=65537, key_size=2048)))
            with self.assertRaises(installer.InstallError):
                installer.install_runtime(config, github_key, allocation_key, prefix=root, expected_uid=os.getuid())
    ...
    def test_staged_package_has_isolated_import_closure(self) -> None:
        ...
        provision = (ROOT / "scripts/host/provision-wsl-jit-contract.sh").read_text(encoding="utf-8")
        self.assertIn("check_delivery.py", provision)
        self.assertIn("install-outbound-worker-runtime.py", provision)
        unit = (ROOT / "packaging/systemd/self-hosted-ci-outbound-worker.service").read_text(encoding="utf-8")
        self.assertIn("ConditionPathExists=/etc/self-hosted-ci/outbound-worker.runtime-ready", unit)
        self.assertIn("ExecCondition=/usr/local/lib/self-hosted-ci/install-outbound-worker-runtime.py --verify", unit)
