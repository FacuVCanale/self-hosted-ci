...
class IncusBoundaryInstallerTests(unittest.TestCase):
    def test_installer_is_plan_only_and_apply_requires_exact_acknowledgements(
        self,
    ) -> None:
    ...
    def test_task_is_one_shot_password_limited_and_credentials_are_rotated(
        self,
    ) -> None:
        source = INSTALLER.read_text(encoding="utf-8")
        for token in (
    ...
    def test_full_payload_is_verified_in_run_and_bounded_by_systemd(self) -> None:
        ...
        for token in (
            ...
            "systemctl kill failed with exit code",
    ...
    def test_payload_initializes_once_and_builds_exact_dedicated_boundary(self) -> None:
        ...
        for token in (
            ...
            'root disk path=/ pool="${pool}" size=12GiB',
    ...
    def test_payload_rejects_host_facing_devices_uplink_nat_and_extra_disks(
        self,
    ) -> None:
        source = PAYLOAD.read_text(encoding="utf-8")
        for token in (
            ...
            '"forbidden_devices":false',
            '"bridge_uplink":false',
            '"ipv4_nat":false',
            '"ipv6_nat":false',
            '"project_restricted":true',
            '"project_instance_limit":1',
            '"storage_driver":"dir"',
            '"storage_filesystem":"ext4"',
            '"storage_filesystem_uuid":"%s"',
            '"storage_project_quota":true',
            '"storage_mount_persistent":true',
            '"storage_quota_canary_passed":true',
            '"storage_mount_unit_enabled":true',
            '"storage_mount_unit_active":true',
            '"incus_mount_ordering_verified":true',
            '"storage_mount_root_owned":true',
            '"storage_pool_size":"16GiB"',
            '"storage_image_apparent_bytes":%s',
            '"storage_image_allocated_bytes":%s',
    ...
    def test_payload_proves_policy_with_negative_canaries_and_cleans_them(self) -> None:
        source = PAYLOAD.read_text(encoding="utf-8")
        for token in (
            "expect_policy_rejection privileged",
            "security.privileged=true",
            "expect_policy_rejection nesting",
            "security.nesting=true",
            'incus config device add "${canary_proxy}" proxy0 proxy',
            "negative canary unexpectedly succeeded: proxy",
            "expect_policy_rejection idmap",
    ...
    def test_boundary_rejects_extra_projects_and_canary_residue(self) -> None:
        ...
        for token in (
            ...
            self.assertIn(token, source)
        self.assertLess(
            source.index("cleanup_owned_canaries\n"),
            source.index("preflight found an existing Incus instance"),
        )

    def test_boundary_uses_only_the_ext4_loop_backend(self) -> None:
        combined = INSTALLER.read_text(encoding="utf-8") + PAYLOAD.read_text(
            encoding="utf-8"
        )
        for token in (
            'storage_driver = "dir"',
            'storage_filesystem = "ext4"',
            "storage_project_quota",
            "storage_mount_persistent",
            "storage_mount_root_owned",
        ):
    ...
    def test_boundary_installer_has_no_external_product_or_runner_surface(self) -> None:
        combined = (
            INSTALLER.read_text(encoding="utf-8") + PAYLOAD.read_text(encoding="utf-8")
        ).lower()
        for forbidden in (
            "cloud" + "flare",
            "github.com",
