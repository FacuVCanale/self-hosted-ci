...
def lifecycle(mode: str) -> dict:
    return {
        "jit": True, "ephemeral_registration": True, "jobs_started": 1,
        ...
        "terminal_mode": mode, "orphan_registrations": 0,
    }
...
def host_evidence() -> dict:
    return {
        ...
        "checks": [
            {"id": item, "status": "pass", "evidence_refs": [f"evidence/{item}.json"], "notes": ""}
            for item in sorted(REQUIRED_CHECKS)
        ],
        "observed_artifact_kinds": [],
        "network_policy": {
            "default_deny": True, "management_separated": True,
            ...
            "proxy_only_egress": True, "private_dns_fail_closed": True,
            "loaded_before_registration": True, "windows_reboot_verified": True, "wsl_reboot_verified": True,
        },
...
def boundary() -> dict:
    value = {
        ...
        "components": [
            {"id": item, "status": "verified", "artifact_digest": "sha256:" + "a" * 64, "evidence_refs": [f"evidence/{item}.json"]}
            for item in sorted(REQUIRED_COMPONENTS)
        ],
        "host_security": host_evidence(),
        "network_policy": {
            "network_policy_version": 2, "enabled": True, "default_egress": "deny",
            "blocked_cidrs": sorted(BLOCKED_NETWORK_CIDRS),
            "blocked_endpoints": ["windows-host", "windows-gateway", "metadata", "management-plane", "reviewer", "control-plane", "deploy", "container-engine-sockets", "incus-api"],
            "allowed_destinations": ["github-actions-egress-proxy"],
            "dns_private_or_rebound_resolution": "deny",
            "must_load_before_runner_registration": True, "must_survive_reboot": True,
        ...
        "measurements": {
            ...
            "artifacts": [{
                "ref": "placeholder", "sha256": "0" * 64, "size": 0,
                "mode": "0000", "uid": 0, "gid": 0,
            }],
        },
    }
    return value
...
def materialize(directory: Path, *, activation: bool = True) -> tuple[dict, Path]:
    ...
    refs.update(
        ref for check in value["host_security"]["checks"] for ref in check["evidence_refs"]
    )
    for ref in refs:
        path = directory / ref
        ...
        if ref == "evidence/network-policy.json":
            ...
            path.write_text(json.dumps({"observed": ref, "version": "pinned-v1"}), encoding="utf-8")
        path.chmod(0o640)
    template = directory / "template.json"
    output = directory / "boundary.json"
    template.write_text(json.dumps(value), encoding="utf-8")
    result = subprocess.run([
        sys.executable, str(COLLECT), "--input", str(template), "--output", str(output),
        "--measurement-root", str(directory),
...
def verifier_command(path: Path) -> list[str]:
    return [
        sys.executable, str(VERIFY), "--evidence", str(path),
        "--reviewer-public-key", str(path.parent / "reviewer-public-key.pem"),
...
class RunnerBoundaryV2Tests(unittest.TestCase):
    def test_complete_bundle_enables_and_schema_validates(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            schema = json.loads((ROOT / "schemas/runner-boundary-v2.schema.json").read_text())
            Draft202012Validator(schema).validate(value)
            measured, blockers = verify_host_measurements(value, Path(directory))
            verify_runner_boundary_attestation(
                value, REVIEWER_PRIVATE.public_key(),
                pinned_fingerprint=REVIEWER_FINGERPRINT,
            )
            decision = evaluate_runner_boundary(
                value, measured_component_digests=measured, measurement_blockers=blockers,
    ...
    def test_activation_component_network_and_host_gaps_block(self) -> None:
        mutations = (
            (lambda value: value.__setitem__("activation_requested", False), "activation-not-requested"),
            (lambda value: value["components"][0].__setitem__("status", "unverified"), "component-unverified"),
            (lambda value: value["network_policy"].__setitem__("enabled", False), "network-policy-not-enabled"),
            (lambda value: value["network_policy"]["blocked_endpoints"].remove("incus-api"), "network-policy-blocked-endpoints-incomplete"),
            (lambda value: next(item for item in value["components"] if item["id"] == "network-policy").__setitem__("artifact_digest", "sha256:" + "f" * 64), "component-host-measurement-digest-mismatch:network-policy"),
            (lambda value: value["host_security"]["checks"][0].__setitem__("status", "unverified"), "host:check-unverified"),
        )
        with tempfile.TemporaryDirectory() as directory:
            for mutate, expected in mutations:
                value, _ = materialize(Path(directory)); value.pop("attestation"); mutate(value)
                value = sign_runner_boundary(value, REVIEWER_PRIVATE)
                measured, measurement_blockers = verify_host_measurements(value, Path(directory))
                decision = evaluate_runner_boundary(
                    value, measured_component_digests=measured,
    ...
    def test_readiness_cli_exit_codes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(verifier_command(path), text=True, capture_output=True)
    ...
    def test_host_measurements_fail_closed_on_bytes_version_acl_and_policy_drift(self) -> None:
        mutations = (
            lambda root: (root / "evidence/garm.json").write_text("mutated bytes"),
            lambda root: (root / "evidence/runner-image.json").write_text('{"version":"changed"}'),
            lambda root: (root / "evidence/incus.json").chmod(0o600),
            lambda root: (root / "evidence/network-policy.json").write_text('{"enabled":false}'),
        )
        for mutate in mutations:
            with self.subTest(mutation=mutate), tempfile.TemporaryDirectory() as directory:
                ...
                self.assertEqual(3, result.returncode, result.stdout + result.stderr)
                self.assertTrue(
                    any("measurement" in blocker for blocker in json.loads(result.stdout)["blockers"])
                )

    def test_arbitrary_status_files_cannot_enable_without_pinned_reviewer_signature(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            path.write_text('{"runner_boundary_version":2,' + canonical[1:], encoding="utf-8")
            result = subprocess.run(verifier_command(path), text=True, capture_output=True)
            self.assertEqual(2, result.returncode, result.stdout + result.stderr)
    ...
    def test_provisioning_is_plan_only_by_default_and_apply_is_guarded(self) -> None:
        syntax = subprocess.run(["bash", "-n", str(PROVISION)], text=True, capture_output=True)
        ...
        apply = subprocess.run(["bash", str(PROVISION), "--apply"], text=True, capture_output=True)
        ...
        self.assertIn('/usr/local/lib/self-hosted-ci/verify-wsl-jit-readiness.py', source)
    ...
    def test_systemd_units_require_boundary_and_activation_sentinels(self) -> None:
        ...
        self.assertIn("--evidence /etc/self-hosted-ci/runner-boundary-v2.json", verifier)
        self.assertIn("Requires=self-hosted-ci-boundary-verify.service incus.service", service)
        self.assertIn("ExecStartPre=+/usr/local/lib/self-hosted-ci/verify-wsl-jit-readiness.py", service)
        self.assertIn(
