...
REQUIRED_COMPONENTS = frozenset({"windows-account", "wsl-distro", "incus", "garm", "network-policy", "runner-image"})
...
def verify_runner_boundary_attestation(
    value: Mapping[str, Any], public_key: ed25519.Ed25519PublicKey, *,
    ...
    if not isinstance(attestation, Mapping) or set(attestation) != {
        ...
        raise RunnerBoundaryError("runner boundary attestation requires exact v1 fields")
    ...
    if (
        ...
        raise RunnerBoundaryError("runner boundary signer does not match pinned reviewer key")
    payload = {key: item for key, item in value.items() if key != "attestation"}
    try:
        verify_detached(
            payload, attestation.get("signature"), public_key,
            domain=BOUNDARY_ATTESTATION_DOMAIN,
        )
    except ValueError as exc:
        raise RunnerBoundaryError("runner boundary attestation signature is invalid") from exc


def verify_host_measurements(value: Mapping[str, Any], evidence_root: Path) -> tuple[dict[str, str], tuple[str, ...]]:
    ...
    for item in artifacts:
        ...
        if not isinstance(item, Mapping) or set(item) != required:
            ...
            continue
        ref = item["ref"]
        if not isinstance(ref, str) or not ref or PurePosixPath(ref).is_absolute() or ".." in PurePosixPath(ref).parts:
        ...
        if (
            not isinstance(item["sha256"], str) or not _HEX_DIGEST.fullmatch(item["sha256"])
            or not isinstance(item["size"], int) or item["size"] < 0
            or not isinstance(item["mode"], str) or not re.fullmatch(r"[0-7]{4}", item["mode"])
    ...
    if isinstance(host, Mapping) and isinstance(host.get("checks"), list):
        for check in host["checks"]:
            if isinstance(check, Mapping) and check.get("status") == "pass":
                ...
                if not isinstance(refs, list) or any(ref not in observed for ref in refs):
                    blockers.append(f"host-check-measurement-missing:{check.get('id', 'unknown')}")

    network_bytes = canonicalize_jcs(value.get("network_policy"))
    network_refs = [
        ref for component in components or []
        if isinstance(component, Mapping) and component.get("id") == "network-policy"
        for ref in component.get("evidence_refs", [])
    ]
    if not any(
        ref in observed and root.joinpath(*PurePosixPath(ref).parts).read_bytes() == network_bytes
...
def evaluate_runner_boundary(
    value: Mapping[str, Any], *, measured_component_digests: Mapping[str, str] | None = None,
    ...
    required = {"runner_boundary_version", "activation_requested", "components", "host_security", "network_policy", "measurements", "attestation"}
    if not isinstance(value, Mapping) or set(value) != required or value.get("runner_boundary_version") != 2:
    ...
    for item in components:
        if not isinstance(item, Mapping) or set(item) != {"id", "status", "artifact_digest", "evidence_refs"}:
        ...
        if item["status"] != "verified" or not isinstance(item["artifact_digest"], str) or not _DIGEST.fullmatch(item["artifact_digest"]):
        ...
        if measured_component_digests.get(component_id) != item["artifact_digest"]:
            blockers.append(f"component-host-measurement-digest-mismatch:{component_id}")
    ...
    if not isinstance(network, Mapping) or set(network) != network_required or network.get("network_policy_version") != 2:
        ...
        required_endpoints = {
            "windows-host", "windows-gateway", "metadata", "management-plane",
            "reviewer", "control-plane", "deploy", "container-engine-sockets", "incus-api",
        }
        if not isinstance(endpoints, list) or not required_endpoints.issubset(endpoints):
        ...
        if network["must_load_before_runner_registration"] is not True or network["must_survive_reboot"] is not True:
    ...
    return RunnerBoundaryDecision(not unique, "verified" if not unique else "blocked", unique)
