...
_WORKFLOW_REF = re.compile(
    r"([A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+)/\.github/workflows/ci-jit-pilot-child\.yml@refs/heads/([A-Za-z0-9._/-]+)"
)
...
class JitPilotPackageV1:
    ...
    @classmethod
    def from_mapping(
        cls, value: Mapping[str, Any], *, now: datetime
    ) -> JitPilotPackageV1:
    ...
    def validate(self, *, now: datetime) -> None:
        ...
        if not isinstance(self.base_branch, str) or not re.fullmatch(
            r"[A-Za-z0-9._/-]+", self.base_branch
        ):
            raise JitPilotError("pilot base branch is invalid")
        for field in ("base_sha", "head_sha", "tested_merge_sha"):
            if not isinstance(getattr(self, field), str) or not _SHA.fullmatch(
                getattr(self, field)
            ):
                raise JitPilotError(f"{field} must be a full lowercase SHA")
        workflow = (
            _WORKFLOW_REF.fullmatch(self.workflow_ref)
            if isinstance(self.workflow_ref, str)
            else None
        )
        if (
            workflow is None
            or workflow.group(1) != self.repository
            or workflow.group(2) != self.base_branch
        ):
        ...
        if self.backend == "local":
            ...
            if str(allocation) != self.allocation_id or not _LABEL.fullmatch(
                self.runner_label or ""
            ):
            ...
            if self.allocation_id is not None or self.runner_label is not None:
                raise JitPilotError(
                    "hosted pilot cannot carry local allocation authority"
                )
        else:
        ...
        if (
            ...
            or expires <= issued
            or (expires - issued).total_seconds() > 900
        ):
...
class GitHubApiReader:
    ...
    def __init__(
        self, package: JitPilotPackageV1, token: str, *, opener=urllib.request.urlopen
    ) -> None:
    ...
    def _get(self, path: str) -> Mapping[str, Any]:
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "self-hosted-ci-jit-pilot/1",
            },
        )
        try:
        ...
        try:
            ...
            def exact_object(pairs):
                ...
                for key, item in pairs:
                    if key in result:
                        raise JitPilotError(
                            "pilot GitHub response contains duplicate keys"
                        )
                    result[key] = item
                return result
    ...
    def workflow(self):
        return self._get(
            f"/repos/{self.package.repository}/actions/workflows/ci-jit-pilot-child.yml"
        )
...
def revalidate_package(package: JitPilotPackageV1, github: PilotGitHubReader) -> None:
    ...
    if (
        pull.get("number") != package.pr_number
    ...
    if (
        workflow.get("path") != ".github/workflows/ci-jit-pilot-child.yml"
...
class PilotBroker(Protocol):
    ...
    def prove_clean(
        self, allocation_id: str, runner_label: str
    ) -> Mapping[str, Any]: ...
...
class PilotTerminalMonitor:
    ...
    def monitor(
        self, *, allocation_id: str, runner_label: str, run_id: int, job_id: int
    ) -> str:
        try:
        ...
        if (
            str(parsed_allocation) != allocation_id
            or not _LABEL.fullmatch(runner_label)
            or any(
        ...
        while conclusion is None:
            ...
            matches = [
                ...
                if (
                    ...
                    and job.get("name") == "local-quality"
            ...
            if job.get("status") == "completed":
                ...
                if run.get("status") != "completed":
                    if (
                        run.get("status") not in {"queued", "in_progress"}
                        or run.get("conclusion") is not None
                    ):
                        raise JitPilotError(
                            "terminal monitor observed an invalid run state"
                        )
                ...
                break
            if (
                job.get("status") not in {"queued", "in_progress"}
                or job.get("conclusion") is not None
            ):
                raise JitPilotError(
                    "terminal monitor observed an invalid nonterminal state"
                )
            if (
                run.get("status") not in {"queued", "in_progress"}
                or run.get("conclusion") is not None
                ...
                raise JitPilotError(
                    "terminal monitor observed run completion before its exact job"
                )
            if self.monotonic() >= deadline:
                conclusion = "timeout"
                break
        ...
        if unexpected:
            raise JitPilotError(
                "unexpected GitHub job conclusion was cleaned as failure"
            )
        return conclusion
...
def validation_main(
    ...
    *,
    ...
    try:
        ...
        if env.get("GITHUB_REPOSITORY") != package.repository or env.get(
            "GITHUB_REPOSITORY_ID"
        ) != str(package.repository_id):
            raise JitPilotError("workflow event repository crossed the pilot package")
        revalidate_package(
            package, GitHubApiReader(package, env.get("GITHUB_TOKEN", ""))
        )
        ...
        outputs = {
            ...
            "pr_number": str(package.pr_number),
            "base_sha": package.base_sha,
            "head_sha": package.head_sha,
            "tested_merge_sha": package.tested_merge_sha,
