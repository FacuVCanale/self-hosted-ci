    ...
    def __post_init__(self) -> None:
        if any(
            ...
            raise WorkerAuthorityError(
                "worker App numeric identities must be positive integers"
            )
...
class WorkerInstallationToken:
    ...
    def assert_current(self, authority: WorkerAppAuthorityV1, now: datetime) -> None:
        if authority != self.authority or now.tzinfo is None or self.expires_at <= now:
            raise WorkerAuthorityError(
                "worker installation token is expired or crossed authority"
            )
...
class RootPrivateKeySigner:
    ...
    def from_file(cls, path: Path) -> RootPrivateKeySigner:
        try:
            ...
            try:
                ...
                if (
                    ...
                    raise WorkerAuthorityError(
                        "worker App private key must be root-owned regular 0600"
                    )
                key_bytes = os.read(descriptor, 65_537)
                if len(key_bytes) != info.st_size:
                    raise WorkerAuthorityError(
                        "worker App private key changed while reading"
                    )
            finally:
                os.close(descriptor)
        ...
        if not isinstance(key, rsa.RSAPrivateKey) or key.key_size < 2048:
            raise WorkerAuthorityError(
                "worker App private key must be RSA 2048-bit or stronger"
            )
        return cls(key)
...
class WorkerGitHubClient:
    ...
    def authenticate(self) -> WorkerInstallationToken:
        ...
        installation = self._json(
            self._request(
                "GET",
                f"/repos/{self.authority.repository}/installation",
                app_headers,
            ),
            200,
        )
        ...
        if any(
            installation.get(key) != value
            for key, value in expected_installation.items()
        ):
    ...
    def repository(self, token: WorkerInstallationToken) -> Mapping[str, Any]:
        value = self._token_json(
            "GET", f"/repos/{self.authority.repository}", token, 200
        )
        if (
    ...
    def pull_request(
        ...
        value = self._token_json(
            "GET", f"/repos/{self.authority.repository}/pulls/{number}", token, 200
        )
    ...
    def merge_commit_parents(
        self, sha: str, token: WorkerInstallationToken
    ) -> tuple[str, ...]:
        if not isinstance(sha, str) or not _SHA.fullmatch(sha):
            raise WorkerAuthorityError("GitHub merge commit SHA is invalid")
        value = self._token_json(
            "GET",
            f"/repos/{self.authority.repository}/git/commits/{sha}",
            token,
            200,
        )
    ...
    def workflow(self, token: WorkerInstallationToken) -> Mapping[str, Any]:
        ...
        value = self._token_json(
            "GET",
            f"/repos/{self.authority.repository}/actions/workflows/{workflow}",
            token,
            200,
        )
        if (
            value.get("path") != self.authority.workflow_path
    ...
    def dispatch_pilot(self, pilot_package: str, token: WorkerInstallationToken) -> int:
        ...
        if self.authority.workflow_id != "ci-jit-pilot-child.yml":
            raise WorkerAuthorityError(
                "pilot dispatch requires the fixed pilot workflow"
            )
        return self._dispatch_exact("pilot_package", pilot_package, token)

    def _dispatch_exact(
        self, input_name: str, package: str, token: WorkerInstallationToken
    ) -> int:
        ...
        if (
            ...
            or not package
            or len(package.encode()) > 60_000
        ):
            raise WorkerAuthorityError("workflow package is absent or oversized")
        workflow = quote(self.authority.workflow_id, safe="")
        value = self._token_json(
            "POST",
            f"/repos/{self.authority.repository}/actions/workflows/{workflow}/dispatches",
            token,
            200,
            {
        ...
        if (
            ...
            or run_id < 1
            or value.get("run_url")
            != f"{API_ROOT}/repos/{self.authority.repository}/actions/runs/{run_id}"
        ):
    ...
    def run(self, run_id: int, token: WorkerInstallationToken) -> Mapping[str, Any]:
        ...
        value = self._token_json(
            "GET",
            f"/repos/{self.authority.repository}/actions/runs/{run_id}",
            token,
            200,
        )
        if (
    ...
    def jobs(self, run_id: int, token: WorkerInstallationToken) -> Mapping[str, Any]:
        ...
        value = self._token_json(
            "GET",
            f"/repos/{self.authority.repository}/actions/runs/{run_id}/jobs?filter=latest&per_page=100",
            token,
            200,
        )
        if (
            set(value) != {"total_count", "jobs"}
        ...
        if value["total_count"] != len(value["jobs"]) or any(
            not isinstance(job, Mapping) or job.get("run_id") != run_id
            for job in value["jobs"]
        ):
            raise WorkerAuthorityError(
                "workflow jobs crossed or truncated the exact run"
            )
        return value
    ...
    def _token_json(
        ...
        method: str,
        path: str,
        token: WorkerInstallationToken,
        status: int,
        body: Mapping[str, object] | None = None,
    ) -> Mapping[str, Any]:
        token.assert_current(self.authority, self._clock())
        return self._json(
            self._request(method, path, self._headers(token.value), body), status
        )

    def _parse_token(
        self, value: Mapping[str, Any], now: datetime
    ) -> WorkerInstallationToken:
        ...
        try:
            ...
            raise WorkerAuthorityError(
                "worker installation token expiry is invalid"
            ) from exc
        ...
        return WorkerInstallationToken(
            token, expires_at - TOKEN_SAFETY_MARGIN, self.authority
        )
    ...
    def _headers(bearer: str) -> Mapping[str, str]:
        return {
            ...
            "User-Agent": "self-hosted-ci-worker/1",
        }
        ...
        try:
            ...
            def exact_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
                ...
                for key, item in pairs:
                    if key in result:
                        raise WorkerAuthorityError(
                            "GitHub worker response contains duplicate keys"
                        )
                    result[key] = item
                return result
            ...
            raise WorkerAuthorityError(
                "GitHub worker response is invalid JSON"
            ) from exc
