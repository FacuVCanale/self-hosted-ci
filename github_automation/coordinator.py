...
class ReservePartialFailure(CoordinatorUnavailable):
    ...
    def __init__(self, allocation_id: str, message: str = "allocation reserve partially applied"):
...
class ChildDispatchAdapter(Protocol):
    ...
    def observe_exact_job(self, run_id: int, runner_label: str) -> ObservedWorkflowJob: ...
...
def _assert_exact_recovery(
    recovered: Mapping[str, str], allocation_id: str,
) -> None:
...
def github_hosted_fallback(package: Mapping[str, Any]) -> ProtocolPackage:
    ...
    value = dict(package)
    value.update(backend="github", execution_trust_mode="github-hosted",
                 allocation_id=None, allocation_nonce=None, runner_label=None)
    for field in (
        ...
        "attestation_envelope_digest", "attestation_request_linkage_hash", "local_admission_id",
        "local_admission_digest", "local_evidence_id", "local_evidence_digest", "local_result_kind",
        "local_child_run_id", "local_child_job_id", "started_test_marker_digest",
        "canonical_command_digest", "terminal_at", "check_outbox_idempotency_key",
    ):
        value[field] = None
    return ProtocolPackage.from_mapping(value)
...
def outbound_local_dispatch(
    package: Mapping[str, Any], reservation: Mapping[str, Any], *,
    allocation: LocalAllocationAdapter, github: ChildDispatchAdapter,
    ...
    try:
        reserved = allocation.reserve(reservation) if should_reserve else persisted_reserved
        ...
        if not isinstance(allocation_id, str) or exc.allocation_id != allocation_id:
            raise CoordinatorUnavailable("partial reserve failure crossed the allocation") from exc
    ...
    if reserved["allocation_id"] != reservation.get("allocation_id") or reserved["runner_label"] != reservation.get("scale_set_name"):
        raise CoordinatorUnavailable("allocation reserve response crossed the reservation")
    if progress is not None:
        progress.reserved(reserved)
    local = dict(package)
    local.update(allocation_id=reserved["allocation_id"], allocation_nonce=reservation.get("nonce"), runner_label=reserved["runner_label"])
    ...
    try:
        ...
        if resumed_run_id is None:
            if resume.get("phase") == "dispatching":
                raise CoordinatorUnavailable("GitHub dispatch receipt is ambiguous; refusing redispatch")
            if progress is not None:
                progress.dispatching()
            dispatch_receipt_ambiguous = True
            dispatch_run_id = github.dispatch_package(protocol.values)
            if not isinstance(dispatch_run_id, int) or isinstance(dispatch_run_id, bool) or dispatch_run_id <= 0:
                raise CoordinatorUnavailable("GitHub dispatch receipt is invalid")
            if progress is not None:
                progress.dispatched(dispatch_run_id)
            dispatch_receipt_ambiguous = False
        else:
            if not isinstance(resumed_run_id, int) or isinstance(resumed_run_id, bool) or resumed_run_id <= 0:
                raise CoordinatorUnavailable("persisted GitHub dispatch receipt is invalid")
        ...
        if observed.run_id != dispatch_run_id:
            raise CoordinatorUnavailable("observed job is not bound to the dispatch run")
        if progress is not None:
            progress.observed(observed)
        payload = dict(reservation)
        payload.pop("allocation_reservation_version", None)
        payload.update(runner_allocation_version=1, run_id=str(observed.run_id),
                       run_attempt=observed.run_attempt, job_id=str(observed.job_id),
                       job_name=observed.job_name, dispatch_sha=observed.dispatch_sha,
        ...
        if resume.get("phase") == "finalizing" and "finalized" not in resume:
            raise CoordinatorUnavailable("allocation finalize receipt is ambiguous; refusing replay")
        if progress is not None:
            progress.finalizing()
        finalized = allocation.finalize(envelope)
        if finalized.get("allocation_id") != reserved["allocation_id"] or finalized.get("state") != "enabled-awaiting-claim":
        ...
        if progress is not None:
            if dispatch_receipt_ambiguous or (resume.get("phase") == "dispatching" and resumed_run_id is None):
