...
class AppAuthorityV1:
    ...
    def __post_init__(self) -> None:
        ...
        if dict(self.permissions) != MINIMUM_APP_PERMISSIONS:
            raise ControlFailure(
                "App permissions must be exactly metadata:read/checks:write"
            )
    ...
    def mint(self, identity: RuntimeIdentity, request: MintRequest) -> MintRequest:
        ...
        if (
            not request.memory_only
            or not request.masked
            or request.export_to_children
            or request.persist
        ):
    ...
    def rotated_to(self, successor: "AppAuthorityV1") -> "AppAuthorityV1":
        ...
        if self.key_state != "active" or successor.key_state != "active":
            raise ControlFailure(
                "rotation requires active old and new keys before switch"
            )
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolFailure(
                "dispatch workflow_id must be one exact workflow file or ID"
            )
        ...
        if self.ref != self.default_branch:
            raise ProtocolFailure(
                "child workflow ref must be the trusted default branch"
            )
...
def parse_observed_workflow_job(
    ...
    *,
    ...
    if not isinstance(runner_label, str) or not re.fullmatch(
        r"wsl-jit-[0-9a-f]{32}", runner_label
    ):
    ...
    if (
        set(jobs) != {"total_count", "jobs"}
        or not isinstance(jobs["total_count"], int)
    ...
    if len(matches) != 1:
        raise ProtocolFailure(
            "dispatch exposed multiple jobs with the exact local identity"
        )
    job = matches[0]
    attempt = run.get("run_attempt")
    job_id, job_name = job.get("id"), job.get("name")
    _positive_int(attempt, "run_attempt", ProtocolFailure)
    _positive_int(job_id, "workflow_job_id", ProtocolFailure)
...
def parse_dispatch_response(
    request: DispatchRequest, status: int, body: Mapping[str, Any]
) -> int:
    ...
    if set(body) != {"workflow_run_id", "run_url", "html_url"}:
        raise ProtocolFailure(
            "dispatch response schema must contain only workflow_run_id, run_url and html_url"
        )
    ...
    if body.get("run_url") != expected_run_url:
        raise ProtocolFailure(
            "dispatch run_url is not bound to the exact repository and run"
        )
    if body.get("html_url") != expected_html_url:
        raise ProtocolFailure(
            "dispatch html_url is not bound to the exact repository and run"
        )
    return run_id
...
_PROTOCOL_FIELDS = frozenset(
    {
        ...
        "execution_trust_policy_version",
        "repository_id",
        "repository",
        "pr_number",
        "logical_key",
...
class ProtocolPackage:
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "ProtocolPackage":
        ...
        if missing or unknown:
            raise ProtocolFailure(
                f"protocol fields mismatch; missing={missing}, unknown={unknown}"
            )
        data = dict(value)
        for field in (
            "protocol_version",
        ...
        for field in (
            ...
            if not isinstance(data[field], str) or not _SHA.fullmatch(data[field]):
                raise ProtocolFailure(
                    f"{field} must be a full lowercase 40-character SHA"
                )
        if (
            data["tested_sha"] != data["tested_merge_sha"]
            or data["check_target_sha"] != data["tested_merge_sha"]
        ):
            raise ProtocolFailure(
                "PR tested/check target SHA must equal exact synthetic merge SHA"
            )
        expected_key = (
            f"{data['repository_id']}:{data['pr_number']}:{data['head_sha']}:ci-gate"
        )
        if data["logical_key"] != expected_key:
            raise ProtocolFailure(
                "logical_key does not bind repository/PR/head/ci-gate"
            )
        ...
        if not isinstance(data["policy_version"], str) or not _SHA.fullmatch(
            data["policy_version"]
        ):
            raise ProtocolFailure("policy_version must be an exact commit SHA")
        if not isinstance(data["key_manifest_digest"], str) or not _SHA256.fullmatch(
            data["key_manifest_digest"]
        ):
        ...
        if not isinstance(data["missing_source_ids"], list) or data[
            "missing_source_ids"
        ] != sorted(set(data["missing_source_ids"])):
        ...
        if backend == "local":
            if not isinstance(data["allocation_id"], str) or not _UUID.fullmatch(
                data["allocation_id"]
            ):
                raise ProtocolFailure(
                    "local backend requires a reserved allocation identity"
                )
            if not isinstance(data["allocation_nonce"], str) or not _NONCE.fullmatch(
                data["allocation_nonce"]
            ):
                raise ProtocolFailure(
                    "local backend requires the reserved allocation nonce"
                )
            if not isinstance(data["runner_label"], str) or not re.fullmatch(
                r"wsl-jit-[0-9a-f]{32}", data["runner_label"]
            ):
                raise ProtocolFailure(
                    "local backend requires the unique reserved runner label"
                )
            ...
            if any(data[field] is None for field in _ATTESTATION_FIELDS):
                raise ProtocolFailure(
                    "local backend requires complete attestation authority fields"
                )
            for field in (
                "key_manifest_digest_at_issuance",
                "attestation_public_key_fingerprint",
                ...
                if not isinstance(data[field], str) or not _SHA256.fullmatch(
                    data[field]
                ):
            ...
            if any(
                ...
                for field in ("allocation_id", "allocation_nonce", "runner_label")
            ):
                raise ProtocolFailure(
                    "hosted dispatch cannot carry local allocation authority"
                )
            if data["execution_trust_mode"] != "github-hosted":
                raise ProtocolFailure(
                    "github backend requires github-hosted trust mode"
                )
            if any(data[field] is not None for field in _ATTESTATION_FIELDS):
                raise ProtocolFailure(
                    "hosted dispatch cannot carry attestation authority"
                )
            local_fields = {
                "local_admission_id",
                "local_admission_digest",
            ...
            if any(data[field] is not None for field in local_fields):
                raise ProtocolFailure(
                    "hosted dispatch cannot cross-bind local result fields"
                )
        else:
            raise ProtocolFailure("backend must be local or github")
        return cls(data)

    def assert_current_tuple(
        self,
        *,
        repository_id: int,
        repository: str,
        ...
        if any(self.values[key] != wanted for key, wanted in expected.items()):
            raise ProtocolFailure(
                "protocol package is stale or for a different exact tuple"
            )
...
def validate_ci_gate_source(
    event: Mapping[str, Any], *, authority: AppAuthorityV1, check_target_sha: str
) -> bool:
...
def validate_formal_claim(
    *,
    ...
    job = jobs[0]
    if (
        job.get("id") != data["local_child_job_id"]
    ...
    *,
    ...
    try:
        ...
        execution_deadline = _parse_time(
            hosted.get("execution_deadline"), "execution_deadline"
        )
    ...
    if any(gate.get(key) != value for key, value in exact_gate.items()):
        return False
    if not (
        hosted.get("tested_sha")
        == hosted.get("check_target_sha")
        == data["tested_merge_sha"]
    ):
        return False
    ...
    if hosted.get("workflow_ref") != data["default_branch"]:
        return False
    if hosted.get("run_id") != gate.get("hosted_run_id") or hosted.get(
        "job_id"
    ) != gate.get("hosted_job_id"):
        return False
    if (
        hosted.get("trustworthy_github_hosted") is not True
        or terminal_at > execution_deadline
    ):
        return False
